...
void GridGraph_Individually(int width,int height,int num_pixels,int num_labels)
{
    ...
    float *result = new float[num_pixels];   // stores result of optimization
	...
	try{
		...
		for ( int l1 = 0; l1 < num_labels; l1++ )
			for (int l2 = 0; l2 < num_labels; l2++ ){
                int cost = 0;//(l1-l2)*(l1-l2) <= 4  ? (l1-l2)*(l1-l2):4;
...
void GridGraph_DArraySArraySpatVarying(int width,int height,int num_pixels,int num_labels)
{
    ...
    float *V = new float[num_pixels];
    float *H = new float[num_pixels];
...
void GeneralGraph_DArraySArray(int width,int height,int num_pixels,int num_labels)
{
	...
	for ( int l1 = 0; l1 < num_labels; l1++ )
		for (int l2 = 0; l2 < num_labels; l2++ )
            smooth[l1+l2*num_labels] = 0;//(l1-l2)*(l1-l2) <= 4  ? (l1-l2)*(l1-l2):4;
	...
	try{
        ...
        for (int y = 0; y < height; y++ )
            for (int  x = 1; x < width; x++ )
                gc->setNeighbors(x+y*width,x-1+y*width,0);

        // next set up vertical neighbors
        for (int y = 1; y < height; y++ )
            for (int  x = 0; x < width; x++ )
                gc->setNeighbors(x+y*width,x+(y-1)*width,0);
