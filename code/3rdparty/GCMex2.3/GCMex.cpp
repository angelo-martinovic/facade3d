    ...
    )
{
  ...
  if(mxGetNumberOfDimensions(in[IN_UNARY]) != 2 ||
     mxGetClassID(in[IN_UNARY]) != mxDOUBLE_CLASS)
    mexErrMsgTxt("Unary term must be a matrix of class DOUBLE");

  if(mxGetClassID(in[IN_LABELCOST]) != mxDOUBLE_CLASS)
    mexErrMsgTxt("Labelcost term must be a matrix of class DOUBLE");
