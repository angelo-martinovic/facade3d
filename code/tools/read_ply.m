function [vertex,vertexNormal,vertexColor,face,faceColor] = read_ply(filename,type)
% READ_PLY - read data from PLY file.
%   [VERTEX,NORMAL,COLOR,FACE,FCOLOR] = READ_PLY(FILENAME,TYPE) reads a
%   point cloud from FILENAME, and returns n x 3 matrices of vertices 
%   VERTEX, vertex normals NORMAL, and vertex colors COLOR. 
%
%   If TYPE='mesh', the script additionally reads the m x 3 matrices of
%   faces FACE and face colors FCOLOR. The entries in FACE are indices to
%   rows of VERTEX.
    dl = DispatchingLogger.getInstance();
    
    dl.Log(VerbosityLevel.Debug,sprintf(' - - Reading PCL from %s.\n',filename));    
    [d,~] = plyread(filename);

    %% Vertex positions
    if isfield(d.vertex,'x')
        vertex = [d.vertex.x, d.vertex.y, d.vertex.z];
        dl.Log(VerbosityLevel.Debug,sprintf(' - - - Read %d vertices.\n',size(vertex,1)));
    else
        dl.Log(VerbosityLevel.Debug,sprintf(' - - - Vertex positions not found.\n'));
        vertex = [];
    end
    ...
    if isfield(d.vertex,'nx')
        vertexNormal = [d.vertex.nx, d.vertex.ny, d.vertex.nz];
    else
        dl.Log(VerbosityLevel.Debug,sprintf(' - - - Vertex normals not found.\n'));
        vertexNormal = [];
    end
    ...
    if strcmp(type,'mesh')
        if isfield(d,'face')
            if isfield(d.face,'vertex_indices')
                face = cellfun(@(x)(x+1),d.face.vertex_indices,'UniformOutput',false);
                face = cell2mat(face);
                dl.Log(VerbosityLevel.Debug,sprintf(' - - - Read %d faces.\n',size(face,1)));
            else
                dl.Log(VerbosityLevel.Debug,sprintf(' - - - Face vertex indices not found.\n'));
            end
