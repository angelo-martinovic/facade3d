...
if isfield(d.vertex,'x')
    ...
    fprintf('Read %d vertices.\n',size(vertex,1));
else
    warning('Vertex positions not found');
    vertex = [];
end
...
elseif isfield(d.vertex,'diffuse_red')
    ...
    warning('Vertex colors not found');
    vertexColor = [];
end
...
%     warning('Vertex normals not found');
    vertexNormal = [];
end
...
if strcmp(type,'mesh')
    if isfield(d,'face')
        if isfield(d.face,'vertex_indices')
            ...
            face = cell2mat(face);
            fprintf('Read %d faces.\n',size(face,1));
        else
            warning('Face vertex indices not found');
        end

        if isfield(d.face,'red')
            faceColor = [d.face.red, d.face.green, d.face.blue];
        else
            warning('Face colors not found');
        end
    end
end
