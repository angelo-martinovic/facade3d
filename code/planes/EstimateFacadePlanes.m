...
%     VisualizePointCloud2(points,normals,colors,1);
    ...
    %% For each camera, fit a plane to the point cloud
    newLabels = zeros(length(points),length(cameras),'int8');
%     depths = zeros(length(points),length(cameras));
    facadePlanesP = zeros(length(cameras),3);   % Mean inlier point
    facadePlanesN = zeros(length(cameras),3);   % Plane normal
    facadePlanesB = zeros(length(cameras),6);   % Inlier bounding box
    
    nIter = length(cameras);
    ...
    parfor i=1:nIter
        ...
        [p_fPlane,n_fPlane,b_fPlane] = FitPlane(pointSubset(1:10:end,:));
       ...
       % p_fPlane=p_fPlane_avg;
        ...
        facadePlanesP(i,:) = p_fPlane;
        facadePlanesN(i,:) = n_fPlane;
        facadePlanesB(i,:) = b_fPlane;
...
%         dlmwrite(depthFileName,img,' ');
    ...
    facadePlanes.p = facadePlanesP;
    facadePlanes.n = facadePlanesN;
    facadePlanes.b = facadePlanesB;
    save('facadePlanes.mat','facadePlanes');