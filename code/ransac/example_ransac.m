...
X = points(:,1);
    ...
    Z = points(:,3);
skip=10;
scatter3(X(1:skip:end),Y(1:skip:end),Z(1:skip:end),20*ones(length(X(1:skip:end)),1),'k','fill');
...
d=4*length(p_tot)/5;%number of nearby points required
 
%Initialize samples to pick from
samples_pick=[1:1:length(p_tot)];
p_pick=p_tot;
for i=1:1 %Search for 4 planes
