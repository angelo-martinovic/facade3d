...
int EDISON::meanShift(int action)
{
  ...
  float *modes;
  int *mpc;
  iProc.GetRegions(&labels,&modes,&mpc);
  ...
  int *lbls = iProc.GetLabels();
  for(unsigned int i=0;i<height_;i++)
  {
      for(unsigned int j=0;j<width_;j++)
          fprintf(fp, "%d ",lbls[i*width_+j]);
  ...
  /*
  iProc.BuildRAM();
  int* adjMatrix = iProc.GetRegionAdjacencyMatrix();
 ...
 for(unsigned int i=0;i<numRegions;i++)
  {
      for(unsigned int j=0;j<numRegions;j++)
          fprintf(fp,"%d ",*(adjMatrix+i*numRegions+j));
